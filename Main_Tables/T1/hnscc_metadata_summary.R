...
setwd("/projects/b1198/epifluidlab/ravi/hnscc_pembro_cs131_trial/Main_Tables/T1")
file_path <- '../../Data/Demographics_HNSCC_CP.csv'
df <- read_csv(file_path)

selected_columns <- c("Subject ID", "Site", "Age", "Gender", "Race", "Ethnicity", "Smoking", "Alcohol", "Primary Disease Site", "Tumor Stage (Initial Diagnosis)")
df_selected <- df[selected_columns]
df_selected <- distinct(df_selected)
selected_columns <- c("Site", "Age", "Gender", "Smoking", "Alcohol", "Race", "Ethnicity", "Primary Disease Site", "Tumor Stage (Initial Diagnosis)")
